# app/core/fund_manager.py
"""
Quản lý Quỹ — tạo nhóm, tham gia, xem thành viên.
...
Cách dùng:
    ...
    # Tạo nhóm
    result = fm.create_group("Quỹ Nguyễn")
...
class FundManager:
    ...
    def create_group(self, name: str) -> dict:
        ...
        with get_connection() as conn:
            # Cho phép tạo nhiều nhóm quỹ
            # Chỉ giới hạn tên nhóm không trùng lặp nếu cần, nhưng hiện tại không bắt buộc
    ...
    def leave_group(self, group_id: int) -> dict:
        ...
        with get_connection() as conn:
            ...
            if group["owner_username"] == username:
                return {
                    "success": False,
                    "message": "Bạn là chủ quỹ, không thể rời. "
                               "Hãy dùng 'Giải tán quỹ' để xóa quỹ."
                }
        ...
        return {"success": True, "message": f"Đã rời quỹ '{group['name']}'"}

    def disband_group(self, group_id: int) -> dict:
        """Giải tán nhóm (chỉ owner). Xóa cả thành viên."""
        username = session.username
        with get_connection() as conn:
            group = conn.execute(
                "SELECT * FROM family_groups WHERE id=? AND owner_username=?", 
                (group_id, username)
            ).fetchone()
            if not group:
                return {"success": False, "message": "Bạn không phải chủ quỹ này"}
        ...
        return {"success": True, "message": f"Đã giải tán quỹ '{group['name']}'"}
    ...
    def get_members(self, group_id: int) -> list[dict]:
        """Lấy danh sách thành viên của nhóm cùng với số tiền đã góp quỹ."""
        with get_connection() as conn:
            rows = conn.execute("""
                SELECT 
                    gm.username, 
                    ...
                    gm.joined_at,
                    CASE gm.role 
                        WHEN 'owner'  THEN 'Chủ quỹ' 
                        WHEN 'member' THEN 'Thành viên' 
                        ELSE gm.role 
                    END AS role_display,
                    COALESCE((
                        SELECT SUM(amount) FROM transactions 
                        WHERE owner_username = gm.username AND type = 'income'
    ...
    def get_member_transactions(self, username: str) -> list[dict]:
        """Lấy lịch sử giao dịch (thu nhập/góp quỹ) của thành viên."""
        with get_connection() as conn:
            rows = conn.execute("""
                SELECT * FROM transactions
                WHERE owner_username = ? AND type = 'income'
