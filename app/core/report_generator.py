...
"""
Xuất báo cáo tài chính tháng ra PDF bằng ReportLab.
Bao gồm: tổng kết thu chi, biểu đồ danh mục, bảng giao dịch, dự báo AI.
"""
...
class ReportGenerator:
    ...
    def _metric_cell(self, label: str, value: float,
        ...
        lbl = Paragraph(f'<font size="8" color="#888888"><b>{label}</b></font>',
                        styles["normal"])
        val_color = color.hexval() if hasattr(color, "hexval") else "#333333"
        # ReportLab color to hex
        try:
    ...
    def _build_transaction_table(self, data: dict) -> list:
        ...
        for tx in txs[:50]:  # Giới hạn 50 dòng để PDF không quá dài
            try:
